...
const VoiceAssistant = () => {
  const { message, publishMessage } = useMqtt("", "esp8266/led");
  ...
  const isSpeakingRef = useRef(false); // Ref to track if speech synthesis is in progress
  ...
  const speak = (text: string) => {
    ...
    isSpeakingRef.current = true; // Set speaking flag to true

    utterance.onend = () => {
      console.log("Finished speaking, restarting listening...");
      isSpeakingRef.current = false; // Set speaking flag to false
      setTimeout(() => {
        restartListening();
      }, 500); // Delay to prevent recognizing its own voice
    };
  ...
  useEffect(() => {
    ...
    if (recognizedText && !isSpeakingRef.current) {
      stopListening();
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      recognizer.onend = () => {
        ...
        if (!stopListenRef.current && !isSpeakingRef.current) {
          console.log("restart");
          restartListening();
        } else {
  ...
  return (
    <div style={{ padding: "20px", fontFamily: "Arial, sans-serif" }}>
      ...
      <button onClick={() => toggleLed(message === "1" ? "0" : "1")}>
